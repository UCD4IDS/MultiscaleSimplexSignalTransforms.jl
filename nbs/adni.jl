...
# ╔═╡ a699efb2-bf41-11ed-1954-99866c5b0c19
using Revise

# ╔═╡ c9e8e00e-e636-4758-8d2b-7e90478cb53c
using MultiscaleSimplexSignalTransforms

# ╔═╡ 54e1d8a7-37f2-4744-abbd-e49ed74711ef
using Graphs, Plots, Chain, Arpack, LinearAlgebra, StatsBase, Distances, Unzip, SimpleWeightedGraphs, Roots, Polynomials

# ╔═╡ e0ddbc17-69a8-4257-92e0-b17eef988eb5
using Meshes, MeshViz

# ╔═╡ 12d7b2b3-6d59-4ae9-ac34-a2bf47c8bdc7
using FileIO, MeshIO, Colors

# ╔═╡ 6203c640-1366-4b18-92fc-33ea052407d0
using Makie, WGLMakie

# ╔═╡ 0cbeb68f-256a-4f71-ab3a-2e8a7fc56fe3
using DelimitedFiles

# ╔═╡ 901d620d-c2e2-4964-b475-7f6a9a260632
import GeometryBasics: value, Mesh, coordinates, faces

# ╔═╡ d4545c17-a2f2-4fa8-b1c5-68d8c1c17766
function MultiscaleSimplexSignalTransforms.SimplexTree(v::AbstractVector{Int})
    tree = SimplexTree()
    for i ∈ v
        MultiscaleSimplexSignalTransforms.child!(tree, i)
    end
    tree
end

# ╔═╡ 888b0e69-f538-4696-bc79-7697751abead
...
# ╔═╡ fd41567d-755d-44a8-a77e-1314cdc3ad22
function MultiscaleSimplexSignalTransforms.SimplexTree(mesh::Meshes.SimpleMesh)
	a = SimplexTree(mesh.topology.elems)
	for face ∈ mesh.topology.connec
		insert!(a, face.indices)
	end
	a
end

# ╔═╡ db112218-f0f2-402e-be93-aefb1874b91b
Meshes.SimpleMesh(mesh::Mesh) = Meshes.SimpleMesh(
	Meshes.Point3.(coordinates(mesh)),
	connect.(Tuple.(broadcast.(value, faces(mesh))))
)

# ╔═╡ 9cd99e42-b272-4ad7-a812-8a9f8466e962
function readnlines(io, n)
	lines = IOBuffer()
	for _ ∈ 1:n
		write(lines, readline(io; keep=true))
	end
	seekstart(lines)
end

# ╔═╡ d35d7542-6374-450f-9b6d-b4e373d1988e
read_asc(fn) = readdlm(fn, Float64)[:,end]

# ╔═╡ 60d2f936-1a36-40dc-b294-cc10f47ade04
adnipath = "/Users/eug/Desktop/ADNI"

# ╔═╡ fbecb2b9-00fe-49c3-9d1a-df4ad3215936
getmesh(relpath, basepath=adnipath) = load(joinpath(basepath, relpath)) |>
	geomesh -> (SimplexTree(geomesh), Meshes.SimpleMesh(geomesh))

# ╔═╡ af8b163b-2723-4087-a90e-49a79348d843
MappedMesh = @NamedTuple{mesh::Meshes.SimpleMesh{3,Float64}, thick::Vector{Float64}}

# ╔═╡ 88bf2ab8-cff2-49b9-8cdf-d1c0cfa1ef8e
read_off(fns)::MappedMesh = (;
	mesh = open(fns[1]) do io
		readline(io) # throw away the header
		nvert, nface = readdlm(IOBuffer(readline(io)), Int)
		mcoor = readdlm(readnlines(io, nvert), Float64)
		mface = readdlm(readnlines(io, nface), Int)[:,2:end] .+ 1
		Meshes.SimpleMesh(
			Meshes.Point3.(eachrow(mcoor)),
			connect.(Tuple.(eachrow(mface)))
		)
	end,
	thick = read_asc(fns[2])
)

# ╔═╡ 75b75ced-cde2-40fa-9a07-c2e1d3343f14
walk_adnidir(adnipath, subdir) = @chain adnipath begin
	joinpath(subdir)
	walkdir
	collect
	filter(x -> isempty(x[2]), _)
	[joinpath(r, f) for (r, _, fs) ∈ _ for f ∈ fs]
end

# ╔═╡ ace9f9be-1581-498b-b315-69f17e18100b
function get_adni(adnipath)
	mesh_fns = walk_adnidir(adnipath, "surfaces")
	thick_fns= walk_adnidir(adnipath, "thickness")
	meshes = (read_off(fns) for fns ∈ zip(mesh_fns, thick_fns))
end

# ╔═╡ c86d96c3-75ac-4618-984c-e468e267ab6a
meshes = get_adni(adnipath)

# ╔═╡ 745f2dda-ae56-434d-90ab-0d861ef3caa9
amesh = first(meshes)

# ╔═╡ c99ec16c-2502-467b-a2fc-3d397a657f2a
# reg = KRegion(SimplexTree(load(".obj")), 2)

# ╔═╡ a23ddceb-93e2-420e-a899-2060981586c6
# part = kGHWT(reg)

# ╔═╡ 8f5b69fc-3e62-473c-a597-38ab2c43de14


# ╔═╡ aadce4b1-4a9c-44c8-bc37-df8bd429a850
# t, pltmesh = getmesh(joinpath("surfaces", "AD", "011_S_0003", "2005-09-01_10_42_12.0", "lh.off"))

# ╔═╡ e7245783-fb88-4b69-a6c3-b2fe094ddbcf
# mesh = @chain begin
# 	load("/Users/eug/Downloads/Toysmith_Windem_Up_Flippin_Animals_Dog/meshes/model.obj")
# 	Meshes.SimpleMesh(
...
# ╔═╡ 843a0be5-068e-4f70-94ba-a9e9cf444ba7
# dog = load("/Users/eug/Downloads/texture.png")

# ╔═╡ 9e382aa2-be1c-4c4a-ae1d-63472241f061


# ╔═╡ 1ad7e164-8b16-46c9-8fb5-b3c996678bd4
# viz(mesh, color=dog, axis=(; show_axis=false))
...
Graphs = "~1.8.0"
MultiscaleSimplexSignalTransforms = "~0.4.0"
Makie = "~0.19.2"
MeshIO = "~0.4.10"
MeshViz = "~0.7.4"
Meshes = "~0.28.0"
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "e0c7476b7d05b4d12d2c94b21136d89ea1ad59be"
...
deps = ["Arpack", "Chain", "Clustering", "DataStructures", "EnumX", "GraphIO", "Graphs", "Infiltrator", "InvertedIndices", "IterativeSolvers", "LaTeXStrings", "Lazy", "LinearAlgebra", "Match", "MetaGraphs", "ParserCombinator", "PersistentCohomology", "Plots", "Roots", "SimpleWeightedGraphs", "SparseArrays", "StatsBase", "Unzip"]
git-tree-sha1 = "6fbb46f1ebcd395010d9556a7e0e10608f7b1dfb"
...
git-tree-sha1 = "0b8a71b1c14c41faecc6fc07be25499d38f5fe63"
...
git-tree-sha1 = "5fd465f9452e9de27c3d8743d9abfe7115a93662"
...
git-tree-sha1 = "413b13d6d4acef267b1c64dcb4abe653ec92a24c"
