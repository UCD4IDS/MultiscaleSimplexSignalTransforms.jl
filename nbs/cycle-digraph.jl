...
# ╔═╡ 4a91d272-dbb7-11ec-015a-59d6681a6cb7
using Graphs, Plots, GraphRecipes, LinearAlgebra, MetaGraphs, MultiscaleSimplexSignalTransforms, Arpack

# ╔═╡ ffa3f6f2-3bd2-4e89-9173-da893eaca72f
n = 50

# ╔═╡ b4017162-88fe-49e5-a84d-a8edc1cfbe35
g = cycle_digraph(n)

# ╔═╡ ac79c5cc-de0b-4bbd-a17a-55f12a012492
L = hodgelaplacian(g; trimap=(ae; edgeset)->false) |> Matrix

# ╔═╡ 105af819-f483-4619-aa87-80a2c7cf1a9a
(Λ, X) = eigen(L)

# ╔═╡ 8bcec718-be50-492b-bd88-4c20b74c8e40
3 |> nrand -> 
plot(
	X'*hcat(
		ones(n),
		# [i> n/2 ? -1 : 1 for i=1:n],
		[i <=12 || i > 38 ? -1 : 1 for i=1:n],
		rand((-1, 1), (n, nrand))
	) .|> abs;
	labels=hcat(
		"directed cycle",
		"pair of paths",
		fill("random orientation", nrand)...
	),
	title="Magnitude of L₁ eigenvector coefficients",
	lw=[[2 2] fill(1, 1, nrand)]
)

# ╔═╡ ad32ed0c-957a-43bf-8d41-79b9fe524ea9
plot(X[:,1:2]; label=false)

# ╔═╡ 93800990-0348-4906-bd5f-b799c319d892
argmin(abs.(X[:,2] |> x -> deleteat!(x, 12)))
...
GraphRecipes = "~0.5.9"
Graphs = "~1.5.0"
MultiscaleSimplexSignalTransforms = "~0.2.0"
MetaGraphs = "~0.7.1"
Plots = "~1.29.0"
"""
...
git-tree-sha1 = "bac99955efc8f4a9aae7aae10508b0de6dc02473"
