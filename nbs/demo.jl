...
# ╔═╡ 6d38a5d0-9e92-11ed-001a-0db092f34e14
begin
	using Pkg
	dev = Pkg.develop
	dev("MultiscaleSimplexSignalTransforms")
end

# ╔═╡ 757c40a4-3456-4357-87ca-bb9335a8bad7
using Revise

# ╔═╡ 33190f43-88f8-4795-bce6-035670c46596
using MultiscaleSimplexSignalTransforms

# ╔═╡ 8089401e-1f96-4f51-9410-e215e6bdea0e
using SparseArrays, LinearAlgebra, StatsBase, Arpack, Graphs, Plots, Chain, DataStructures, MAT, GraphRecipes

# ╔═╡ 5acf0238-6e0c-4ee7-9d46-83635151a21d
using SimpleWeightedGraphs

# ╔═╡ 1784b17e-ab63-42df-aff1-2384211835ec
using DataFrames

# ╔═╡ 2ebf7859-7df9-40d2-affc-333a7fcbbea7
k=2

# ╔═╡ 17aa6728-4da4-4208-97cc-ed57b36b8a9a
lap_from_adj(M, k) = KRegion(cliquecomplex(Graph(M)), k; weakadjweight=1.0) |>
	reg -> (
		k_laplacian(
			reg;
...
# ╔═╡ ce7b2241-cf20-45c9-a3a7-ef2c7294b09d
lap_from_adj(erdos_renyi(10, .5), 1)

# ╔═╡ b88c287e-71b9-4337-a9e9-b14f7b8b7d84
Is = (1, 1, 3)

# ╔═╡ bfd252c7-d260-4a94-a25d-de27d0fbd4ab
function tricycle(n)
...
# ╔═╡ 07c6274c-bfcb-4cfb-b4d5-f9199a9b5ac8
dg, xs, ys = tricycle(11)

# ╔═╡ 02243c97-0497-4fae-bd94-8f2c8e9bf789
g = Graph(dg)

# ╔═╡ 504b06fe-7d68-4f4f-b73e-f2a0ce77aa62
reg = KRegion(cliquecomplex(g), k; weakadjweight=1.0);

# ╔═╡ 180270e2-080e-42f6-a752-09b21cab6504
part = kGHWT(
	reg;
	normalization=Normalization.Combinatorial
);

# ╔═╡ 6430af46-d90a-4124-a7bc-150b89ca0374
klap = k_laplacian(reg; normalization=Normalization.Symmetric)

# ╔═╡ bb9ce5de-f5f1-4404-af71-0f863cc2e763
_, X = eigs(klap+I, nev=2, which=:SM)

# ╔═╡ 5bc33796-6e87-4e05-91a2-cd6b94067753
fpart = FullPartition(
	cliquecomplex(g),
	(; complex, subregion_inds) -> KRegion(complex, k; subregion_inds, weakadjweight=1.0)
) |> kGHWT;

# ╔═╡ 7382851d-704c-4634-b62c-8e287bd8dc52
splot(reg, part[Is...]; k, xs = Float64.(xs), ys = Float64.(ys), palette=:viridis, size=2)

# ╔═╡ 4b83999a-670c-40da-92c4-0e478750bb07
 splot(
	reg, fpart[Is...]; 
	k, xs=Float64.(xs), ys=Float64.(ys), palette=:viridis, size=2
)

# ╔═╡ cc5be280-2312-496b-9183-7b6301adf04c
@chain begin
	SimpleWeightedGraph(Symmetric(rand(4,4) .* rand(Bool, 4,4)))
	all(adjacency_matrix(_) .== Graphs.weights(_))
end

# ╔═╡ bb655a86-8f03-4a95-81a5-846c82d9d2e4


# ╔═╡ 79e0aeaf-0ef8-4900-a597-545a2914ef84


# ╔═╡ ad5d41ea-9db7-4a2c-8534-65563245245d


# ╔═╡ 167b3984-4725-4f77-a321-9793c98068cd


# ╔═╡ 84caa019-bfa4-4574-9982-90a806e05615


# ╔═╡ bdf2db51-5c12-494f-86a6-c432fbe2687c


# ╔═╡ d4552615-a517-45c1-b4bd-0506795f900d


# ╔═╡ 718d4de6-1a6f-48dc-8655-6a3b17fd2bfd


# ╔═╡ 3d11ac34-d0ae-4774-8c2c-a7abddd2f1a7


# ╔═╡ 2ebecfae-ae6d-46c8-8e5a-b421bc6173aa


# ╔═╡ 0d6e4676-6ac5-4eb3-a4fb-b0d08b16dbb4


# ╔═╡ 9f5539ff-c5e5-4c98-8290-60858f671855


# ╔═╡ f3878248-1a98-40d9-9ca1-923985e71e81


# ╔═╡ fb50acce-7835-4ff3-ad8b-dcd370bcef9d


# ╔═╡ 55d05d48-559e-4237-af24-3fb5f109b49c


# ╔═╡ b028fdae-ce34-48da-ba1c-0f17ef85ab65


# ╔═╡ b444e105-e9cb-4e80-a20c-46284eadf7a3


# ╔═╡ 146e7ae1-1fa4-478e-8496-7a1a31caa4c0


# ╔═╡ 03acf75e-8b95-4cf4-8b01-575040e218b1


# ╔═╡ af9fa98e-0e97-43ed-9cb5-1010cd356952
A = DataFrame(a=rand(Bool,10), b=rand(10))

# ╔═╡ 01ad8bc0-e6bc-4bd6-b0ec-b02a9f400ff6
pairs(groupby(A, :a)) |> collect

# ╔═╡ e1624d5b-580d-4bab-9f38-4cbb663f5827
pairs(groupby(A, :a)) |> collect |> first

# ╔═╡ 053af380-82c8-49a3-88a7-f0bd438fa440
pairs(groupby(A, :a)) |> collect |> first |> first

# ╔═╡ 5790e7c9-c3e0-4e98-acaa-02180b06e843
pairs(groupby(A, :a)) |> collect |> first |> first |> first
...
julia_version = "1.8.2"
manifest_format = "2.0"
project_hash = "f4b5b9141b10cb0dd359f975deefda3b0befc8d5"
...
path = "/Users/eug/.julia/dev/MultiscaleSimplexSignalTransforms"
