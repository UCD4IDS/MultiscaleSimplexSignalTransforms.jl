...
# ╔═╡ af2e6014-44e8-11ed-158b-0349eb1ddef2
begin
	using Pkg
	dev = Pkg.develop
	dev(path="..")
...
# ╔═╡ 41a995f0-3b45-4e5c-af77-07ce2edc2acd
using Revise

# ╔═╡ c06ea53c-3707-4c55-ae81-ce99dd2f3883
using MultiscaleSimplexSignalTransforms

# ╔═╡ 565665ba-7493-445b-a9d2-f4dbf73be7ac
using Graphs, Plots, LinearAlgebra, LaTeXStrings, Arpack, SparseArrays, Clustering, InvertedIndices, DataStructures

# ╔═╡ 15d8b018-b1cd-4c8f-9db4-3f2cd747943d
function tricycle(n)
...
# ╔═╡ 4ea4bb21-5b54-4efc-a8cc-139b7be9f468
...
# ╔═╡ aa88a64c-7cbf-4429-819e-31ad92d24e81
gs = tricycle(5); g, x, y = gs

# ╔═╡ 7d7ac6b1-9c90-408b-a3f1-ca739dc518fa
tris = triangles(g)

# ╔═╡ ec6a96a1-674a-4b81-b8b0-0b54ff269300
tree = hierarchical_partition(g; partfun=klap_part, ksimpfun=triangles)

# ╔═╡ 2f845d61-968e-4528-a007-34b7a15cdf0b
MultiscaleSimplexSignalTransforms.scplot(g::AbstractGraph, x, y; kwargs...) = scplot(triangles(g), x, y; kwargs...)

# ╔═╡ 2527d7e3-27e3-45c6-9a5d-d3cf823d578a
scplot(gs...; tz = treecolors(tree; maxlevel=2))

# ╔═╡ c1dc4f12-896a-4fa1-b296-a7d8d3c19b6f
gp = path_graph(5)

# ╔═╡ bf911d60-6c7c-4d00-b48f-7be809a4e42e
ptree = hierarchical_partition(
	gp; 
	partfun = klap_part, 
	ksimpfun = g -> OneSimplex.(edges(g))
); ghwt_basis!(ptree, 4, 0)

# ╔═╡ 17bd4d10-3a63-44a4-a792-bee5502b3b9d
plot([
	plot([vals.vals zeros(4)]; title="Tag $tag", ylims=[-1,1], legend=false)
	for (tag, vals) in sort(ptree.fvals)
]...)

# ╔═╡ 056acc17-e83c-4a60-89db-194a7cd49101
ghwt_basis(g; partfun=klap_part, ksimpfun=triangles)

# ╔═╡ 2f9fa8c5-522f-443a-a143-88676dd0b055
begin
	ts = [PartitionTree(; sinds = [i]) for i = 1:7]
	ts[1].sinds = 1:7
	lchild!(ts[1], ts[2])
	rchild!(ts[1], ts[3])
	lchild!(ts[2], ts[4])
...
# ╔═╡ 600f1147-bc89-4417-a65b-04bd94d2b899
[!isnothing(c.parent) && c.parent.lchild == c for c in ts]
...
julia_version = "1.8.2"
manifest_format = "2.0"
project_hash = "7db17676c8c3296b0bcbef852a58ef4232c41d75"
