...
# ╔═╡ c0a003b8-7f6f-11ed-1876-9573b9f3e5d6
begin
	using Pkg
	dev = Pkg.develop
	dev(path="..")
...
# ╔═╡ 3737bad3-666b-4731-ab49-a946cc689d38
using Revise

# ╔═╡ fbad2d5b-59bb-413a-b544-712d6f47d80e
using MultiscaleSimplexSignalTransforms

# ╔═╡ f37d302c-c73d-4881-ac5a-4d59790bbeb3
using Graphs, Arpack, Plots, LinearAlgebra, SparseArrays, Clustering, DataStructures, DataFrames, MAT

# ╔═╡ f37e18b8-c200-484e-b258-d435e2ecc428
using MultiscaleGraphSignalTransforms

# ╔═╡ 4dbab504-1d2a-42a2-8980-aa4093e90618
begin
	minnmat = matread("../data/minnesota/minnesota.mat")
	minncoor = minnmat["Problem"]["aux"]["coord"]
	...
	for i ∈ [349, 348] # descending to avoid index counting issues
		rem_vertex!(minnesota.g, i) # remove the node and all associated info in the graph
		...
		minnesota.xs[i] = minnesota.xs[end]
		pop!(minnesota.xs)
		minnesota.ys[i] = minnesota.ys[end]
		pop!(minnesota.ys)
	end

	@assert length(connected_components(minnesota.g)) == 1 "graph is disconnected"
end

# ╔═╡ c22a08d1-6da9-4c8d-bda0-f6ab79899ac7
mcomplex = cliquecomplex(minnesota.g)

# ╔═╡ 13fde94f-2210-46e5-88fe-c182cecd1735
mregion = KRegion(mcomplex, 1; weakadjweight=1.0)

# ╔═╡ 6ebda606-87d8-4974-ac7d-a71ea416cf80
function basisdict(region, do_ghwt)
	part = SubmatrixPartition(
		region;
		basis= do_ghwt ? Basis.Fiedler : Basis.Entire,
		method=PartitionMethod.Kmeans
	)
	basis_dictionary!(part |> (do_ghwt ? kGHWT : kHGLET))
	part
end

# ╔═╡ c88eb0db-3010-46ff-ac41-53ad57d515a9
basisdict(mregion, true)

# ╔═╡ ddc4eb46-b73d-4732-8fa2-0a368c82da2b
function tritest(n, k, do_ghwt)
	gt, xt, yt = triangle_stack(n)
	ct = KComplex(cliquecomplex(Graph(gt), k+1), k; weakadjweight=1.0)
...
# ╔═╡ 42018879-328b-4eb8-b992-b3eb8c2ac42d
tripart, tricomplex, triplot = tritest(20, 1, false)

# ╔═╡ 51c05648-1d95-4044-b7ae-1ef8d49d1a7d
splot(
	tricomplex,
	treecolors(tripart.root; maxlevel=Inf);
	k=1,
	triplot.xs,
	triplot.ys,
	palette=:viridis,
...
# ╔═╡ 4240e6ce-dd6a-4aec-8541-02c433adfbfe
varimax(hcat([tripart.root.fvals[i].vals for i=1:2]...))

# ╔═╡ 631b24f8-ac22-4552-bc3d-0f6f8fabc5ea
plot([
	splot(
		tricomplex,
		v[:];
	...
	for v ∈ (
		eachcol(varimax(hcat([tripart.root.fvals[i].vals for i=1:2]...)))...,
		(tripart.root.fvals[i].vals for i ∈ 1:2)...
	)
]...)

# ╔═╡ cbbe991f-7748-4d45-bf7c-8bad1d5540d0
function pcomplex()
	g, xs, ys = simplex_path(4, 1)
	(
		;
		pc = KComplex(
			cliquecomplex(Graph(g)), 1;
			hullweightfn = t -> 1 ∈ t ? 3.0 : 2.0
		),
		xs, ys
	)
end

# ╔═╡ 3cef07a3-c977-4dd8-844f-e570da5ce105
pc = pcomplex()

# ╔═╡ c2ae7594-3ecf-479e-b385-5e54504d0184
begin
	splot(
		pc.pc,
		ones(n_simp(pc.pc));
	...
	scatter!(pc.xs, pc.ys, ms=20, mc=:white)
	annotate!(pc.xs, pc.ys, 1:length(pc.xs))
end

# ╔═╡ a4b43417-029a-4bd9-9a0d-028bd64cd252
k_laplacian(pc.pc; normalization=Normalization.Weighted, separate=true) |> 
	((Ll, Lu),) -> 
	(Ll, Lu)
	# droptol!(parent(Ll+Lu), 1e-12)
	# (
	# 	filter(>(1e-12), eigen(Matrix(Ll)).values),
	# 	filter(>(1e-12), eigen(Matrix(Lu)).values),
	# 	eigen(Matrix(Ll+Lu)).values
	# )

# ╔═╡ dd59ec7a-be39-46d1-8c8e-197cf419c4ba


# ╔═╡ 5d388850-68e4-4888-bf2f-36691303c291
pc.pc.hullweights

# ╔═╡ 081caf68-65f6-4c2a-9d8c-b2659894ea6e
simplices(pc.pc)

# ╔═╡ d170a7a2-da48-4313-98b5-570c6a74a90e
pc.pc.bdweights

# ╔═╡ 343d5468-2fa7-4d7a-8059-062f4605a59e
[pair for (k, v) in pc.pc.hingemap for pair in v.pairs]

# ╔═╡ fa9f9a37-eee2-4407-84c2-d02b43d96055
...
MAT = "~0.10.3"
MultiscaleGraphSignalTransforms = "~1.7.3"
Plots = "~1.38.0"
Revise = "~3.4.0"
"""
...
julia_version = "1.8.2"
manifest_format = "2.0"
project_hash = "621af451cee47898e1d75e8c94cf69a894c52f80"
...
git-tree-sha1 = "b7903b11d1aa4a96fc63d04de036236099cf8608"
