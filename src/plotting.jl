...
function splot(
	...
	plot!(
		Shape.(e_coor);
		lw= isedge ?
			permutedims(map(v -> v==0 ? size.lw/2 : size.lw, f)) :
			(notribds ? 0 : 1),
		lc = isedge ? c : :black
	)
	scatter!(
		v_xs,
		v_ys;
		ms = isnode ? map(v -> v==0 ? size.ms/2 : size.ms, f) : size.ms,
		alpha = isnode ? 1 : 0,
		c = cgrad(palette),
		msw=0,
		mz = isnode ? (isconst ? fill(0., length(f)) : f) : nothing,
...
function splot(
	...
	scatter!(
		xs,
		ys;
		ms = map(v -> v==0 ? size.ms/2 : size.ms, f),
		alpha = 1,
		c = cgrad(palette),
		msw=0,
		mz = (isconst ? fill(0., length(f)) : f),
		clims = isnothing(clims) ? balancedextrema(f) : clims,
		scatterkwargs...
		# mz = isnode ? asunit(vals) : nothing
	)
